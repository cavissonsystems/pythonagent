...
try:
    ...
    class AsyncHTTPClientProbe(HTTPConnectionProbe):
        def start_exit_call(self, url):
            ...
            return super(AsyncHTTPClientProbe, self).start_exit_call(bt, backend, operation=parsed_url.path)

        def end_exit_call(self, exit_call, future):
            super(AsyncHTTPClientProbe, self).end_exit_call(exit_call, exc_info=future.exc_info())
    ...
    def probe_tornado_httpclient(agent, mod):
        ...
        return AsyncHTTPClientProbe(agent, mod.AsyncHTTPClient).attach('fetch', wrapper_func=None)
except ImportError:
    def probe_tornado_httpclient(agent, mod):
        pass
