...
"""Probing the psycopg2 package.

The probes in here are more complex than for other packages because the
...
class Psycopg2Probe(BaseProbe):
    ...
    def _connect(self, connect, *args, **kwargs):
        ...
        Psycopg2ConnectionProbe(
            self.agent, Connection, Psycopg2CursorProbe).attach('__init__')

        return connect(connection_factory=Connection, *args, **kwargs)


def probe_psycopg2_connection(agent, mod):
    Psycopg2Probe(agent, mod).attach('connect')
