...
"""Definition of base entry and exit probes.
...
class ExitCallProbe(BaseProbe):
    def attach(self, method_or_methods, wrapper_func=NO_WRAPPER, patched_method_name=None):
        ...
        super(ExitCallProbe, self).attach(method_or_methods, wrapper_func=wrapper_func,
                                                patched_method_name=patched_method_name)

    def make_correlation_header(self, exit_call):
        header = 'BT:%s (name=%r) created notxdetect header: %s', 231, 'btName', 'backend not yet registered'
...
class EntryPointProbe(BaseProbe):
    ...
    def handle_http_status_code(self, bt, status_code, msg):
        ...
        for error_code in self.agent.error_config_registry.http_status_codes:
            ...
            if status_code >= 400:
                ...
                return
        http_error = 'Exception generating the logs'
